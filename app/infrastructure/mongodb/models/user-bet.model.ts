...
export interface IUserBet extends Document {
    ...
    historyLength: number; //so these are number of history of the bet, if thhe history number is no the same when looking of it, it means that the bet has been updated and should be found in the history[historyLength]
...
const userBetSchema: Schema = new Schema({
    ...
    historyLength: { type: Number, required: false, default:null, comment:"So these are number of history of the bet, if thhe history number is no the same when looking of it, it means that the bet has been updated and should be found in the history[historyLength]" },
...
export default mongoose.model<IUserBet>('userBet', userBetSchema);