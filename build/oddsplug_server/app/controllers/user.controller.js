...
const updateProfile = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const authService = new authentication_service_1.default();
        const checkEmail = yield authService.getAuthentication({ email: user.email });
        if (checkEmail.length > 0 && !auth._id.equals(checkEmail[0]._id)) {
            return res.status(400).json({ status: false, message: "Email already exists" });
        }
        const checkUsername = yield authService.getAuthentication({ username: user.username });
        ...
        // Update authentication
        const updateAuth = yield authService.updateAuthentication({ _id: auth._id }, { email: user.email, username: user.username, dialingCode: user.dialingCode, mobile: user.mobile, JWT: refreshToken });
        if (!updateAuth) {
