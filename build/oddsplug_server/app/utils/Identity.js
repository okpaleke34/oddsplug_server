...
class Identity {
    constructor() {
        this.generateID = (length, type = "alpha numeral") => {
            let result = '';
            let characters = '';
            if (type == "mix") {
    ...
    getNewID(format) {
        let nextID = '';
        const { staticVal, prevID, isRandom } = format, settings = __rest(format, ["staticVal", "prevID", "isRandom"]);
